        ...
        if categoryindata == "PLAIN_TEXT" and typeindata == "message":
            ...
            if '订阅' == realData:
                ...
                if len(squrl)==0:
                    ...
                    MIXIN_WS_API.sendUserText(ws, conversationId, userId, introductionContent)
                    MIXIN_WS_API.sendUserAppButton(ws, conversationId, userId, "https://mixin-www.zeromesh.net/oauth/authorize?client_id=''''?''''&scope=PROFILE:READ+PHONE:READ+CONTACTS:READ+ASSETS:READ+SNAPSHOTS:READ&response_type=code&return_to='tronado验证端口IP'", "点击授权")
                else:
